...
public partial class App : Application
{
	...
	/// <summary>
	/// Migrates the app data directory from the old "ClaudeMaximus" name to "TessynDesktop".
	/// Must run BEFORE ConfigureLogging and ConfigureServices since both reference the data dir.
	/// </summary>
	private static void MigrateDataDirectory()
	{
		try
		{
			var appData = Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData);
			var oldDir = Path.Combine(appData, "ClaudeMaximus");
			var newDir = Path.Combine(appData, Constants.AppDataFolderName);
		...
		{
			// Best-effort migration — if it fails, the app creates a fresh data dir
		}
	}
